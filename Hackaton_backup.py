...
def fprincipal(prefijox,elementox,cubetax,entradaprocesadax,palabraindividualx,resultadosx,resultadosindicesx,tagx,respuestax):
    prefijox=input("Inserte el numero correspondiente al prefijo de acuerdo a la nomenclatura sistematica:")
    elementox= input("Ingrese el segundo elemento:")
    cubetax= [0 for _ in range(len(palabras))]
    entradaprocesadax= nltk.word_tokenize(elementox)
    entradaprocesadax =[stemmer.stem(palabra.lower()) for palabra in entradaprocesadax]
    for palabraindividualx in entradaprocesadax:
        for i, palabra in enumerate(palabras):
            if palabra == palabraindividualx:
                cubetax[i] = 1
    resultadosx = modelo.predict([numpy.array(cubetax)])
    resultadosindicesx= numpy.argmax(resultadosx)
    tagx=tags[resultadosindicesx]

    for tagaux in datos["contenido"]:
        if tagaux["tag"]== tagx:
            respuestax = tagaux["respuesta"]
...
def mainbot():
    while True:         
        ...
        if lentrada==lhidruros:
            ...
            time.sleep(1.5)
            print(f"{obtsimbolo(elemento)}{prefijo1}{obtsimbolo(elemento3)}{prefijo3}") 
